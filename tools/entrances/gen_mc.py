...
ART = {
    ...
    "boogleswarm": ("art monster/MONSTER_Boogleswarm.webp", None),
    "ketgrin": ("art monster/MONSTER_Ketgrin.webp", None),
    "lurker": ("art monster/MONSTER_Lurker_in_the_Dark.webp", None),
    # ⚠️ NOT THE MODULE'S ART. The three nursery toys were generated by the DM in the book's style and cleaned to
    # transparency (tools/entrances/decheck.py); they live in his Foundry data, not in the module, so their path is
    # absolute-from-Foundry-root and marked with a leading "/" (art_path below). They are also the tokens.
    "raven": ("/mc-portraits/nursery/raven.webp", None),
    "goat": ("/mc-portraits/nursery/goat.webp", None),
    "bunny": ("/mc-portraits/nursery/bunny.webp", None),
    "vweasel": ("art monster/MONSTER_Vermin_Familiar_Weasel.webp", None),
...
def art_path(key):
    """Where a picture lives, as Foundry serves it: the module's art by default, or — with a leading "/" — anywhere
    else under the data folder (the DM's own generated pictures)."""
    f = ART[key][0]
    return f[1:] if f.startswith("/") else TCM + f


def crop_box(key):
    """The page's processing, as fractions of the ORIGINAL file: its explicit crop, then the trim to visible pixels."""
    src, crop = ART[key]
    with Image.open(DATA / art_path(key)) as im:
...
SHORT = {  # a deck key holds about ten letters (deck-play-not-tuning)
    ...
    "boogle": "Boogleswarm", "wisp": "Wisp", "lurker": "The Lurker", "monweasel": "Big weasel", "toys": "Playthings", "crooked": "Crooked Man", "jericho": "Jericho", "harvest": "Harvest", "abbottf": "The Abbot", "jaeger": "Jaeger turns",
...
for n in cards.N:
    ...
    e = {"key": n["key"], "short": SHORT[n["key"]], "name": n["name"].replace("&amp;", "&"), "sub": n["sub"], "classes": classes,
         ...
         "portrait": art_path(re.search(r'src="art/([a-z-]+)\.webp"', n["art"]).group(1)),
...
MATCH = {"boogle": ["Boogleswarm", "Boogle Swarm", "Boogleswarms"],
         # Wisp's token is spelled "Ketgrinn" on the map; the shorter spelling catches both.
         "wisp": ["Wisp", "Ketgrin"], "lurker": ["Lurker in the Dark", "Lurker"],
         # ⚠️ The banner says "A Monstrous Weasel" and the token it reveals is Filthy Jasper — the table is not
         # told whose familiar it is (DM 2026-09-20: "dont use his name").
         "monweasel": ["Filthy Jasper", "Vermin Familiar"],
         "toys": ["Crow", "Goat", "Bunny"],
         "druskenvald": ["Phillip Druskenvald", "Adela Druskenvald"], "sinner": ["Grinning Sinner", "Crossroads"],
...
# THE TOKEN IN THE FLASH (DM 2026-09-19: "for crooked man add a white overlay effect over the token during the lighting"):
# where a banner throws lightning, the NPC's own token on the map flashes white with it (entrances.js flashTokens), on
# the same clock as the banner's bolt — themes2.css @keyframes bolt: 1.1 s, starting 0.33 s in, peaks 0.8 and 0.55.
# ⚠️ IT OUTLIVES THE SCREEN'S BOLT BY HALF A SECOND (DM 2026-09-20: "Leave the flash on the crooked man 0.5 second
# longer than the full screen lightning fade out"). The banner's bolt runs 1.1 s from 0.33 s, so it is dark at 1.43 s;
# the token keeps a low glow through it and fades out by 1.93 s — 1600 ms of its own, same start, same two peaks.
# The Lurker borrows the Crooked Man's bolt entirely — same start, same curve, same half-second overhang.
TOKENFLASH = {"lurker": {"at": 330, "ms": 1600,
                         "curve": [[0, 0], [0.041, 0.8], [0.096, 0.06], [0.151, 0.55], [0.275, 0.22],
                                   [0.688, 0.22], [1, 0]]},
              "toys": {"at": 4600, "ms": 1600,
                       "curve": [[0, 0], [0.041, 0.8], [0.096, 0.06], [0.151, 0.55], [0.275, 0.22],
                                 [0.688, 0.22], [1, 0]]},
              "crooked": {"at": 330, "ms": 1600,
                          "curve": [[0, 0], [0.041, 0.8], [0.096, 0.06], [0.151, 0.55], [0.275, 0.22],
                                    [0.688, 0.22], [1, 0]]}}
# A TROOP ARRIVES ONE AT A TIME (DM 2026-09-20, the nursery toys: "i want them coming in raven goat bunny... each one
# with a sound, as they appear in the intro, show that NPC's token on the map"). Each member's token is unhidden as its
# picture lands in the banner, so the map and the screen always agree about who is here yet; the times are the theme's
# own stagger (themes2.css .t-toys), and the sound at each one is just a clip at the same second (sounds.json).
TROOP = {"toys": [{"at": 0.25, "match": ["Crow"]}, {"at": 1.55, "match": ["Goat"]}, {"at": 2.85, "match": ["Bunny"]}]}
# AN ENTRANCE MAY THROW ONE OF THE MAP'S OWN EFFECTS (same request: "as the intro ends, create a custom lightning flash
# right outside the window and light up all three tokens with an effect"). This is the real §51 one-shot — the quarter
# it strikes from is chosen once by the GM and the window panes on that side light inward — not a banner decoration, so
# it reaches the TV, the DM's monitor and the phones exactly as the storm's own lightning does.
FX = {"toys": [{"id": "lightning", "at": 4.6}]}
...
for e in entries:
    ...
    if e["key"] in TOKENFLASH:
        e["tokenFlash"] = TOKENFLASH[e["key"]]
    if e["key"] in TROOP:
        e["troop"] = TROOP[e["key"]]
    if e["key"] in FX:
        e["fx"] = FX[e["key"]]
