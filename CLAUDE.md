...
**Tested stack (chase upstream, don't freeze — DM 2026-07-26):** last validated: Foundry 14.365 · dnd5e 5.3.3 · midi-qol 14.0.11 · automated-conditions-5e 14.533.15 · midi-item-showcase-community 2.0.2 · DAE 14.0.13 (bench 2026-08-20). **One gap carried forward:** `simplecover5e` is INACTIVE in the bench world, so AC5E's new Simple Cover integration is unvalidated — and it IS active in the DM's campaign world (DESIGN §28.5.4). Beta users will run current releases, so we keep up rather than pin old versions. When the System-health tab flags an unvalidated version: run the combat validation script (DESIGN §28.4), fix what broke, then bump the `TESTED` map in preflight.js AND this line together. Documentation predating this generation is unreliable — check installed module source over training data.
