...
The target picker now flags targets past the activity's reach: the row dims and the distance turns red (still tappable — warnings, not walls). Reach read from `activity.range` (max of reach/value/long, scene units; no reimplementation). The picker header also shows dnd5e's own range label (a chip, e.g. "reach 5 ft" / "80/320 ft") so a red target is self-explanatory. **Verified:** Greatsword (reach 5 ft) → header chip "reach 5 ft"; Gelatinous Cube at 5 ft normal; two Spear Barricades at 20 ft flagged out-of-range.
