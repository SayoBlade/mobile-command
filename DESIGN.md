...
| 28, 28.1–28.12 | Combat hardening; **stack policy §28.4**; ecosystem watch | **Current policy** + fix records; **§28.5.13 = the dnd5e 6.0 gate (2026-09-11): our code ready, midi not; §28.5.14 = AC5E .19 audit + the no-roll advantage route PROVEN and BUILT (2026-09-15; §28.8 retired)** |
...
**BUILT 2026-09-15 (same day as the proof).** `rpc.js attackPreviewBody` now calls AC5E's `_preRollAttack` on a config built the way `AttackActivity#rollAttack` builds one (`buildAttackRollConfig`); nothing rolls. DELETED with it, and the commit says why: the Dice So Nice veto, the `preCreateChatMessage` veto and its 15 s self-expiry, the Automated Animations + Sequencer stubs, the `dnd5e.rollAttack(V2)` hook-array splice, the post-roll message sweep, the 10 s `Promise.race` around the throwaway, the `attackPreviewLatch` (preset.js) with its twists.js guards and the two-tap fire's 12 s wait on it. KEPT: target set/restore, the one-at-a-time chain (targets are shared state), and a try/catch that returns `unevaluated: "ac5e-threw"` for undrawn-token geometry or a renamed export. Phone (`shell.js`): an unevaluated check now renders "Couldn't check advantage — choose it yourself" and no longer resets the player's own adv/dis pick; an evaluated one is unchanged (starred + pre-selected + reasons). Bench 2026-09-15, driven through the executor RPC and then the shell's own picker DOM: identical verdicts to the fake-roll path (advantage / "Target Cannot See Attacker" + "Prone" on the prone bandit; attacker prone → normal; blinded target → advantage), 30–60 ms, ZERO chat cards, ZERO `diceSoNiceRollStart`, ZERO `dnd5e.rollAttack(V2)` firings across six previews incl. two in parallel; prior GM target restored; an injected failure produced the honest banner and recovered on the next toggle. §28.4 leg 3/4 (two-tap hit, parked, damage) re-run clean on the same build.
...
0. **QA pass 2026-09-04 (§28.5.11) — what is still OPEN after the 2026-09-05 fix round:**
   ...
   - ~~**Retire the phantom swing**~~ **BUILT 2026-09-15 (§14 banner, §28.5.14): the fake roll and its ~80 lines of suppression are gone; owed: one live-table glance that no phantom swing shows on the TV.** *(was: DM 2026-09-11: "for now the system makes a 'fake attack' to
...
### 28.5.14 AC5E 14.533.19 (DM updated 2026-09-15 before asking; audit + bench same day) — NOTHING BREAKS, AND THE NO-ROLL ROUTE IS PROVEN → BUILT the same day (§14 banner)
...
> **RETIRED 2026-09-15.** The suppression this section describes hid a throwaway roll; the preview no longer rolls (AC5E asked directly — §14 banner, §28.5.14), so all of it is deleted. Kept here as the record of why it existed.
...
**The phone asks before a crossing (DM 2026-08-07, corrected same day).** FIRST PASS WAS WRONG: it tested only the DESTINATION point, so a token already standing in the vestibule re-prompted on every further step — by which time core had already done the teleport. That is the DM's "you ask DURING teleportation (not before)". The test is now a CROSSING — `region.testPoint(to) && !region.testPoint(from)` — evaluated before anything moves. Verified live on 10.5 Mail Car (Colored): walking in prompts; stepping within the vestibule, walking back out, and wandering outside all stay silent. Copy is the DM's: **"Travel alone to the next cart?"**, with NO destination name — which car is next is something the party should learn by walking into it. Third button **Travel as group** places every player-owned token in the car into the vestibule, so each teleports by core's own route rather than being special-cased; the TV account is excluded (`displayOwnerUser`) and a packed group token counts as the whole party.

**The phone asks before a crossing (superseded note).** The vestibule trigger fires on ENTRY — by design you never open the door — which makes it very easy to cross by accident while shuffling a token. All phone movement funnels through one place, `handleMoveRequest`, so the gate lives there rather than fighting Foundry region hooks: before the update lands, the destination point is tested against enabled `trainDoor` regions (`region.testPoint`, verified true inside / false outside on a live car), and a hit returns `stage: "confirmTeleport"` with the far car's NAME instead of moving. The phone holds the step (dx/dy) and replaces the D-pad with a confirm — replaces, not overlays, because leaving the arrows live invites a stray second tap while deciding — then repeats the exact step with `confirmTeleport: true`. A bare marker region (10.1's locked rear door) has no teleport behaviour and so never prompts. The DM dragging a token on the desktop does not route through here and is unaffected.

**Door census + unlock (DM 2026-08-07: "passenger car is missing teleporters / unlock all locked regions").** Full census run live: every car 10.1–10.8 in BOTH art sets has both doors wired except two deliberate dead ends — **10.1 rear** (the arcane-locked exterior boarding door; there is no car behind the rear car) and **10.8 front** (the locomotive is the Vagrant's). So 10.1 has one teleporter, forward to 10.2, which is correct rather than missing. Its rear door was left with NO teleport behaviour on purpose: an enabled teleport with an unset destination is the Cave A landmine that silently rejects every token move, so a dead end must be a bare region, never a wired one. **Locked walls:** exactly ONE in all 17 train scenes — wall `IZVOQDJKyv2OJmiu` on 10.1 Passenger Car (Colored), at x≈829, sitting inside the rear vestibule (it IS the arcane-locked door). Set LOCKED→CLOSED on the DM's instruction; zero locked doors remain. Stray noted, not touched: an unnamed 1050×490 `Region` on 10.1 Colored with no behaviours.
...
> a journal that's autogenerated, nice design, table of contents, a picture in the index of the PC,
> link to the token sheet, and all the information divided by chapters."*

### 44.1 The premise: this is a COMPILE, not a capture

The instinct is that this needs transcription. It mostly doesn't. Six of the eight chapters below
are a read over records the module already keeps, in structured form, per character — story
answers already carry the question that prompted them, who wrote them, a real timestamp and an
in-world date. Nobody has ever read them back, because there has never been a view.

So the feature is a **view over existing data**, plus one genuinely new recorder (§44.4).

### 44.2 TWO DOCUMENTS, ONE DIRECTION — the rule that keeps it safe

"Player Stories" (§38.4) already exists: one journal, a page per PC, ownership NONE, written by
players from their phones. **The Character File must never extend it — it reads from it.**

One document is authored by players and must never be overwritten; the other is generated and is
overwritten every compile. Merging them means a recompile can eat somebody's backstory. Separate
documents, and the flow only ever runs one way.

### 44.3 Structure

- **One `JournalEntry` per PC** — their own book (DM decision: "per player"). A five-PC party in
  one entry would be forty-odd pages in a single list.
- **A "Character Files" index entry** — a portrait grid, each linking to that PC's book.
- **Ownership: DM only** (`default: NONE`) — DM decision. Note for later: `JournalEntryPage` has
  its OWN ownership field (verified in installed 14.365), so individual chapters can be opened to
  their player without a second document, if that's ever wanted.
- Sheet links use Foundry's native `@UUID[Actor.x]{Name}` enricher — clickable, zero code.
- Foundry's own page list is each book's table of contents; the index is the campaign-level one.

**Chapters, and where each comes from:**

| # | Chapter | Source | Exists today |
|---|---|---|---|
| 1 | **Cover** — portrait, name, class/level, player, link to the sheet | actor | ✅ |
| 2 | **The Character** — species/background/class, level, main weapon, seat | actor + session-zero flags | ✅ |
| 3 | **Backstory** | `system.details.biography` | ✅ |
| 4 | **In Their Own Words** — every story answer under its question, in-world dated | story-journal entries | ✅ |
| 5 | **Creation** — what they chose at session zero and the beat they gave for each step | `charGenBeats` + step-tagged entries | ✅ |
| 6 | **Fate** (Crooked Moon only) — tarot card + what it unlocks, Thread of Fate + touchpoints, Twists, curses | actor flags | ✅ |
| 7 | **Downtime** — activities and the player's own free-text intents | `downtimeState` | ✅ |
| 8 | **Deeds** — the records (§44.4) | NEW recorder | ❌ |
| 9 | **DM's Notes** — hand-written, **never regenerated** | the DM | n/a |
...
spell, and anything cool you can come up with."*
...
- **Nemesis** — the creature that has dealt this PC the most damage across the campaign.
- **Scars** — times dropped to 0 HP, death saves failed and made.
- **Mercy** — healing given. Deliberately included so a support character's chapter isn't empty;
  a file that only measures damage says a cleric did nothing all campaign.
- **Luck** — natural 20s and natural 1s.

**Where the data comes from.** midi workflow completion for damage/attack/usage attribution (the
module already depends on midi throughout), plus the NPC-death detection we ALREADY own — the
skull marker (`markDeadNpcs`) is exactly the "this thing just died" hook. The missing piece is a
`tokenId → last damager` map, kept in memory on the executor.

**Bounded storage — a real constraint, not a nicety.** Actor flags are synced documents; an
unbounded kill list would grow all campaign and ship to every client on every change. So: keep
**running aggregates** (counters, maxima) plus a **capped top-N list** of worthy kills (20),
ranked by `CR − level`. Bounded by construction, and the interesting entries are the ones kept.

**Attribution is an approximation, and the caveat is the honest part.** "Last damager" will
occasionally credit the wrong PC — a fireball that kills three things, a familiar chipping in,
simultaneous damage. Better said now than discovered in six months as a wrong entry that quietly
costs the whole chapter its credibility.

**Nothing before the recorder exists is recoverable.** Chat history could in principle be mined
for past kills; it's fragile and I'd skip it. The chapter starts the day it ships.

**This is a module in its own right** (DM's own observation). Built in its own file with a clean
boundary so it can be lifted out later without surgery.

### 44.5 The regeneration contract — the decision that matters most
...
- The only always-on structure is one in-memory Map of recent damagers.
