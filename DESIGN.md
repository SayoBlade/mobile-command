...
- **Third-party combat HUD leaks onto the phone.** A combat-HUD module (Argon / Enhanced Combat HUD — action/bonus/reaction bars, weapon mastery, Dash/Disengage/Dodge) renders its "<name>'s Turn" popup on phone clients; our Prompt Restyler turned it into a bottom-sheet, and the player was acting through *it* (bypassing Route B → no phone damage tap). **Fix:** `liftDialogAboveShell` now closes apps matching `/argon|enhancedcombat|combat-?hud/` on phone clients only (DM client's shell isn't rendered, so its HUD is untouched), and logs every app rendered over the shell for ID. If the regex misses, the console log names it.
...
- **Resource counters** (Rage, Ki, Action Surge uses, etc.) — *design pending (DM ask 2026-06-15).* Native source is **`item.system.uses`** = `{ value (= max − spent), max, recovery[] }` on each feature item ([dnd5e.mjs:11510](DESIGN.md)), plus legacy `system.resources.{primary,secondary,tertiary}` and spell slots (`system.spells`). Free/limited features (Action Surge) have `uses` but no economy cost. **Open design:** counter-on-action-row vs. a dedicated Resources section vs. both; how to spend/restore from the phone (uses are item-data writes the phone can do). **3rd-party caveat:** exotic/module resources outside `uses`/`resources` are out of scope → DM-from-laptop fallback (warnings-not-walls), with a possible D6 adapter hook later.
